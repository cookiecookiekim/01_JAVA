...
public class FinalFieldTest { // 24-09-09 (월) 5~6교시 클래스와 메서드 파라미터

    /* title. final 키워드에 대해 조금 더 자세히 이해할 수 있다. */
    /* comment. final : 종단의 의미를 가지는 키워드이다.
    ...
    *   7. 클래스 : 상속 불가 */
    ...
    /* comment. 인스턴스가 생성될 때 int 자료형의 기본값인 0이 필드에 들어가는데
    *   초기화 이후 값을 변경할 수 없기 때문에 바로 초기화를 시켜주어야 한다.
    *   그렇지 않으면 compile error가 발생한다. */
//    private final int nonStaticNum;  // compile error 발생
...
//    private static final int STATIC_NUM; // 0이라는 기본값이기 때문에 오류 발생
    private static final double PI = 3.14; // static final 키워드가 동시에 있다면 선언과 동시에 초기화 해야한다.
    /* comment.
    *   static이 붙은 키워드는 생성자를 이용한 초기화 불가능.
    *   생성자는 인스턴스가 생성되는 시점(new를 만나는 시점)에 호출이 되기 때문에
    *   그 전에는 초기화가 일어나지 못한다.
    *   하지만 static 키워드는 프로그램이 run하는 동시에 할당되기 때문에
...
//    }
    /* comment. 정적 초기화 블록으로 초기화 할 수 있다. */ // 초기화 부터 해놓고 싶을 때 사용, 많이 안 쓴단다..
    static {
        STATIC_DOUBLE = 0.5;
}
}
