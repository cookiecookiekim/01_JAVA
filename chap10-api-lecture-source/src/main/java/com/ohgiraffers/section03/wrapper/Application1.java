...
public class Application1 { // 24-09-12 (목) 6교시 Wrapper 클래스
    ...
    public static void main(String[] args) {
        ...
        *   float       Float
         ...
         *   기본(원시)타입을 래퍼클래스로 인스턴스화 하는 것을 Boxing이라고 하며,
         *   래퍼 클래스로 인스턴스화 된 것을 기본 타입으로 변경하는 것을
         *   UnBoxing이라고 한다. */

        int intValue = 20; // 기본 타입
        Integer boxingNumber = new Integer(intValue); // 인스턴스화 - 박싱, 생성자 이용
        Integer boxingNumber2 = Integer.valueOf(intValue); // 클래스명.메서드명(); ← static
        ...
        /* comment. 오토 박싱(Auto Boxing) 오토 언박싱(Auto UnBoxing)
        *   jdk 1.5 부터는 박싱과 언박싱이 필요한 상황에는 컴파일러가
        *   (묵시적)으로 처리해 준다. */

        // 오토 박싱 : 원시타입의 intValue를 자동으로 Integer 타입에 대입
        Integer boxingNumber3 = intValue;

        // 오토 언박싱 : Integer 타입의 boxingNumber3를 원시타입에 대입
